...
res_df <- read_csv("FigureData/fc_mat.csv")
fc_mat <- as.matrix(res_df[,-353])
rownames(fc_mat) <-res_df[[353]]
...
candidates <- reshape2::melt(fc_mat, value.name = "log2fc") %>% dplyr::rename(c("Peptide"="Var1", "sample"="Var2"))
...
HATmeta$sex <- case_when(HATmeta$sex == "Féminin" ~ "Female",
...
colnames(Tbr_df) <- rownames(fc_mat)
...
trainer_tbr <- Tbr_df[train_ind, c(Tbr_sig, ncol(Tbr_df))]
tester_tbr <- Tbr_df[-train_ind, c(Tbr_sig, ncol(Tbr_df))]
...
features <- data.frame(feats = which(lasso_coeff > 0) %>% names())
...
for (i in 1:10){
  ...
  trainer <- Tbr_df[ind, c(Tbr_sig, ncol(Tbr_df))]
  tester <- Tbr_df[-ind, c(Tbr_sig, ncol(Tbr_df))]
  formula <- as.formula(paste("status ~ ", paste(features$feats, collapse = " + ")))
  mod <- train(formula, data = trainer, method = 'glm', metric = 'ROC', trControl = fitControl)
...
for (i in 1:10){
  ...
  tester <- Tbr_df[-ind, c(Tbr_sig, ncol(Tbr_df))]
  formula <- as.formula(paste("status ~ ", paste(features$feats, collapse = " + ")))
  mod <- train(formula, data = trainer, method = 'qda', metric = 'ROC', trControl = fitControl)
  preds <- predict(mod, tester)
  x <- ressummary(preds, tester$status) %>% mutate(index = i)
...
colnames(Tbg_df) <- rownames(fc_mat)
...
trainer_tbg <- Tbg_df[train_ind, c(Tbg_sig, ncol(Tbg_df))]
tester_tbg <- Tbg_df[-train_ind, c(Tbg_sig, ncol(Tbg_df))]
...
# Optimal variables
rfe_optimal <- lmProfile$optVariables
```
...
rfe_optimal <- read_csv("FigureData/Tbg_RFE.csv", col_names = F, show_col_types = F) %>% .$X1 
rf_opt <- match(rfe_optimal, Tbg_sig)
Tbg_df <- Tbg_df[ , c(Tbg_sig, ncol(Tbg_df))]

set.seed(1234)
train_ind <- createDataPartition(Tbg_df$status, p = 0.8, list = F)
trainer_tbg <- Tbg_df[train_ind, c(rf_opt, ncol(Tbg_df))]
tester_tbg <- Tbg_df[-train_ind, c(rf_opt, ncol(Tbg_df))]
...
for (i in 1:10){
  ...
  trainer <- Tbg_df[ind, c(rf_opt, ncol(Tbg_df))]
  tester <- Tbg_df[-ind, c(rf_opt, ncol(Tbg_df))]
  formula <- as.formula(paste("status ~ ", paste(features$feats, collapse = " + ")))
  mod <- train(formula, data = trainer_tbg, method = 'glm', metric = 'ROC', trControl = fitControl)
  preds <- predict(mod, tester)
  x <- ressummary(preds, tester$status) %>% mutate(index = i)
  glm_stats <- rbind(glm_stats, x)
  tbg_model_glm[[i]] <- mod
...
for (i in 1:10){
  ...
  tester <- Tbg_df[-ind, c(rf_opt, ncol(Tbg_df))]
  formula <- as.formula(paste("status ~ ", paste(features$feats, collapse = " + ")))
  mod <- train(formula, data = trainer, method = 'lda', metric = 'ROC', trControl = fitControl)
  preds <- predict(mod, tester)
  x <- ressummary(preds, tester$status)  %>% mutate(index = i)
...
tbg_best <- list(tbg_model_glm[[2]], tbg_model_lda[[6]], tbg_model_boost[[1]], tbg_model_boost[[3]], tbg_model_boost[[5]], tbg_model_mars[[1]], tbg_model_mars[[2]], tbg_model_mars[[3]], tbg_model_mars[[7]], tbg_model_mars[[8]], tbg_model_mars[[9]])
...
tbg_mat <- t(fc_mat[unique(final_tbg$names), ]) %>% as.data.frame()
...
tbr_mat <- t(fc_mat[unique(final_tbr$names), ]) %>% as.data.frame()
...
tbg_diagnostic_t2$stage <- factor(tbg_diagnostic_t2$stage, levels = c("Cn", "Cg", "Pg1", "Pg2", "Cr", "Pr1", "Pr2"))
...
tbr_diagnostic_t2$stage <- factor(tbr_diagnostic_t2$stage, levels = c("Cn", "Cg", "Pg1", "Pg2", "Cr", "Pr1", "Pr2"))
...
***
# Supplemental Figures 6, 7, 8
***

## Permutation Testing

Compare the original ROC values to the distribution of random permuted values. The p-value is the proportion of times the randomized models performed as well or better than our trained model using real data.

### Tbg models

```{r, eval=FALSE}
# write functions to perform permutation tests for various models
# prepare tbg data for permutations
Tbg_perm_df <- data.frame(t(fc_mat[, c(grep("g", colnames(fc_mat)), grep("Cr", colnames(fc_mat)))]))
colnames(Tbg_perm_df) <- rownames(fc_mat)

# add infection binary variable
Tbg_perm_df$sample <- rownames(Tbg_perm_df)
Tbg_status <- data.frame(sample = Tbg_perm_df$sample)
Tbg_status$status <- case_when(grepl("C", Tbg_status$sample ) ~ "Neg",
                               TRUE ~ "Pos") %>% as.factor()

Tbg_perm_df <- merge(Tbg_perm_df, Tbg_status)
Tbg_samples <- Tbg_perm_df$sample
Tbg_perm_df <- Tbg_perm_df %>% dplyr::select(-sample)
rownames(Tbg_perm_df) <- Tbg_samples


permtbg_selected <- function(method_in){
  n_permutations <- 1000
  perm_metrics <- numeric(n_permutations)
  # set control for RFE
  ctrl <- rfeControl(functions = rfFuncs, #rfFuncs is random forest algorithm
                     method = "repeatedcv", #repeated cross-validation approach
                     repeats = 5, # the number of times it should be repeated (k)
  ...
  for(i in 1:n_permutations) {
    Tbg_shuffle <- data.frame(sample = rownames(Tbg_perm_df))
    Tbg_shuffle$status <- case_when(grepl("C", Tbg_shuffle$sample ) ~ "Neg",
                               TRUE ~ "Pos") %>% as.factor()
    # Shuffle the outcome variable
    shuffled_y <- sample(Tbg_shuffle$status)
    Tbg_shuffle$status <-  shuffled_y
    # perform wilcoxon ranked sum test
    Tbg_perm <- reshape2::melt(fc_mat, value.name = "log2fc") %>% 
      dplyr::rename(c("Peptide"="Var1", "sample"="Var2")) %>% inner_join(Tbg_shuffle) %>%
      group_by(Peptide) %>% do(tidy(wilcox.test(log2fc~status, data = ., alternative = "greater")))
    # select the top 2000 peptides by p-value since that's the number used in the true model
    Tbg_perm_sig <- arrange(Tbg_perm, p.value) %>% .$Peptide %>% .[1:2000]
    # train the new model
    rownames(Tbg_shuffle) <- Tbg_shuffle$sample
    # create training dataset partition
    ind <- createDataPartition(Tbg_shuffle$status, p = 0.8, list = F)
    trainer <- Tbg_perm_df[ind, c(Tbg_perm_sig), drop = F] %>% cbind(Tbg_shuffle[ind,"status"]) 
    colnames(trainer)[length(Tbg_perm_sig)+1] <- "status"
    tester <- Tbg_perm_df[-ind, c(Tbg_perm_sig), drop = F] %>% cbind(Tbg_shuffle[-ind,"status"]) 
    colnames(tester)[length(Tbg_perm_sig)+1] <- "status"
    # recursive feature elimination (rfe)
    # define the size of variable subsets the function should consider
    subsets <- c(10, 15, 20, 50, 100, 200, 250)
    lmProfile <- rfe(x=trainer[, -2001], y=trainer$status,
                     sizes = subsets,
                     rfeControl = ctrl)
    # RFE optimal variables
    perm_rfe <- lmProfile$optVariables
    # make new test and train sets using RFE-selected variables
    trainer <- Tbg_perm_df[ind, c(perm_rfe), drop = F] %>% cbind(Tbg_shuffle[ind,"status"]) 
    colnames(trainer)[length(perm_rfe)+1] <- "status"
    tester <- Tbg_perm_df[-ind, c(perm_rfe), drop = F] %>% cbind(Tbg_shuffle[-ind,"status"]) 
    colnames(tester)[length(perm_rfe)+1] <- "status"
    trainer_mat <- model.matrix(status ~ ., data = trainer)[, -1]
    trainer_res <- trainer$status
    y <- as.double(trainer_res) %>% replace(., . == 1, 0) %>% replace(., . == 2, 1)
    tester_mat <- model.matrix(status ~., data = tester)[, -1]
    tester_res <- tester$status
    lasso_out <- glmnet(trainer_mat, trainer_res, alpha = 1, family = 'binomial')
    # find best lambda by cross-validation
    cv_out <- cv.glmnet(trainer_mat, y, alpha = 1)
    bestlam <- cv_out$lambda.min
    lasso_coeff <- predict(lasso_out, s = bestlam, newx = tester, type = 'coefficients')[1:length(perm_rfe), ]
    feature_coeff <- lasso_coeff[-1]
    if (length(feature_coeff[feature_coeff > 0]) > 0) {
    # Train model on shuffled data (use the same parameters as your original model)
    trainer_mat <- trainer_mat[, which(lasso_coeff > 0), drop = F]
    colnames(trainer_mat) <- which(lasso_coeff > 0) %>% names()
    perm_model <- train(x = trainer_mat,
                        y = trainer_res,
                        method = method_in,
                        # we used ROC (AUC) for model selection rather than accuracy or RMSE because our model is meant to predict a binary outcome
                        metric = 'ROC',
                        trControl = fitControl)
      perm_metrics[i] <- perm_model$results$ROC
    } else {
      # Assign ROC = 0.5 if no features pass lasso penalty
      # this is the value of a null, random guessing
      perm_metrics[i] <- 0.5
      }
    }
  return(perm_metrics)
...
# Tbg models that have built-in feature selection


permtbg <- function(method_in, my_model, seed){
  n_permutations <- 1000
  perm_metrics <- numeric(n_permutations)
  # set control for RFE
  ctrl <- rfeControl(functions = rfFuncs, #rfFuncs is random forest algorithm
...
tbg1_metrics <- permtbg_selected('glm')
write_csv(as.data.frame(tbg1_metrics), file = "Tbg_glm_perm_res.txt", col_names = F)
tbg2_metrics <- permtbg('glmboost')
write_csv(as.data.frame(tbg2_metrics), file = "Tbg_boost_perm_res.txt", col_names = F)
tbg3_metrics <- permtbg('earth')
write_csv(as.data.frame(tbg3_metrics), file = "Tbg_MARS_perm_res.txt", col_names = F)
```

## S6A: Plot permutation results for Tbg

```{r}
# calculate p-values from permutation and observed ROC metrics
# we performed 1000 permutations with scrambled target columns
# the real model is counted as one of the permutations so add 1 to the numerator and denominator
tbg1_metrics <- read_csv("FigureData/Permutations/Tbg_glm_perm_res.txt", col_names = F, show_col_types = F) %>% .$X1
summary(tbg1_metrics)
tbg_best[[1]]$results$ROC
sum(tbg1_metrics >= tbg_best[[1]]$results$ROC) + 1 / (1000 + 1)

tbg2_metrics <- read_csv("FigureData/Permutations/Tbg_boost_perm_res.txt", col_names = F, show_col_types = F) %>% .$X1
summary(tbg2_metrics)
max(tbg_best[[3]]$results$ROC)
sum(tbg2_metrics >= max(tbg_best[[3]]$results$ROC)) + 1 / (1000 + 1)

tbg3_metrics <- read_csv("FigureData/Permutations/Tbg_MARS_perm_res.txt", col_names = F, show_col_types = F) %>% .$X1
summary(tbg3_metrics)
max(tbg_best[[10]]$results$ROC)
sum(tbg3_metrics >= max(tbg_best[[10]]$results$ROC)) + 1 / (1000 + 1)


plot_grid(
  data.frame(ROC = tbg1_metrics, method = rep("GLM", 1000)) %>% 
  ggplot(aes(x = ROC)) + 
  geom_histogram(fill = "#1177BB", color = "black") + 
  geom_vline(xintercept = tbg_best[[1]]$results$ROC, lty = 2, color = "red") +
  theme_bw() + 
  scale_x_continuous(limits = c(0,1)) +
  ggtitle("GLM"),
  data.frame(ROC = tbg2_metrics, method = rep("Boost", 1000)) %>% 
  ggplot(aes(x = ROC)) + 
  geom_histogram(fill = "#1177BB", color = "black") + 
  geom_vline(xintercept = max(tbg_best[[3]]$results$ROC), lty = 2, color = "red") +
  theme_bw() + 
  scale_x_continuous(limits = c(0,1)) +
  ggtitle("Boost"),
  data.frame(ROC = tbg3_metrics, method = rep("MARS", 1000)) %>% 
  ggplot(aes(x = ROC)) + 
  geom_histogram(fill = "#1177BB", color = "black") + 
  geom_vline(xintercept = max(tbg_best[[10]]$results$ROC), lty = 2, color = "red") +
  theme_bw() + 
  scale_x_continuous(limits = c(0,1)) +
  ggtitle("MARS"),
  nrow = 1
...
# for tbr models that use lasso selected variables
permtbr_selected <- function(method_in){
  n_permutations <- 1000
  perm_metrics <- numeric(n_permutations)

  for(i in 1:n_permutations) {
    Tbr_shuffle <- data.frame(sample = rownames(Tbr_df))
    Tbr_shuffle$status <- case_when(grepl("C", Tbr_shuffle$sample ) ~ "Neg",
                               TRUE ~ "Pos") %>% as.factor()
    # Shuffle the outcome variable
    shuffled_y <- sample(Tbr_shuffle$status)
    Tbr_shuffle$status <-  shuffled_y
    # perform wilcoxon ranked sum test
    Tbr_perm <- reshape2::melt(fc_mat, value.name = "log2fc") %>% 
      dplyr::rename(c("Peptide"="Var1", "sample"="Var2")) %>% inner_join(Tbr_shuffle) %>%
      group_by(Peptide) %>% do(tidy(wilcox.test(log2fc~status, data = ., alternative = "greater")))
    # select the top 24 peptides by p-value since that's the number used in the true model
    Tbr_perm_sig <- arrange(Tbr_perm, p.value) %>% .$Peptide %>% .[1:24]
    # train the new model
    rownames(Tbr_shuffle) <- Tbr_shuffle$sample
    # create training dataset partition
    ind <- createDataPartition(Tbr_shuffle$status, p = 0.8, list = F)
    trainer <- Tbr_df[ind, c(Tbr_perm_sig), drop = F] %>% cbind(Tbr_shuffle[ind,"status"]) 
    colnames(trainer)[length(Tbr_perm_sig)+1] <- "status"
    tester <- Tbr_df[-ind, c(Tbr_perm_sig), drop = F] %>% cbind(Tbr_shuffle[-ind,"status"]) 
    colnames(tester)[length(Tbr_perm_sig)+1] <- "status"
    trainer_mat <- model.matrix(status ~ ., data = trainer)[, -1]
    trainer_res <- trainer$status
    y <- as.double(trainer_res) %>% replace(., . == 1, 0) %>% replace(., . == 2, 1)
    tester_mat <- model.matrix(status ~., data = tester)[, -1]
    ...
    cv_out <- cv.glmnet(trainer_mat, y, alpha = 1)
    bestlam <- cv_out$lambda.min
    lasso_coeff <- predict(lasso_out, s = bestlam, newx = tester, type = 'coefficients')[1:length(Tbr_perm_sig), ]
    feature_coeff <- lasso_coeff[-1]
    if (length(feature_coeff[feature_coeff > 0]) > 0) {
    # Train model on shuffled data (use the same parameters as your original model)
    trainer_mat <- trainer_mat[, which(lasso_coeff > 0), drop = F]
...
tbr1_metrics <- permtbr_selected('glm')
write_csv(as.data.frame(tbr1_metrics), file = "Tbr_glm_perm_res.txt", col_names = F)
tbr2_metrics <- permtbr('glmboost')
write_csv(as.data.frame(tbr2_metrics), file = "Tbr_boost_perm_res.txt", col_names = F)
tbr3_metrics <- permtbr('earth')
write_csv(as.data.frame(tbr3_metrics), file = "Tbr_MARS_perm_res.txt", col_names = F)
```

## S6B: Plot permutation results for Tbr

```{r}

tbr1_metrics <- read_csv("FigureData/Permutations/Tbr_glm_perm_res.txt", col_names = F, show_col_types = F) %>% .$X1
summary(tbr1_metrics)
tbr_best[[1]]$results$ROC
(sum(tbr1_metrics >= tbr_best[[1]]$results$ROC) + 1)/ (1000 + 1)

tbr2_metrics <- read_csv("FigureData/Permutations/Tbr_boost_perm_res.txt", col_names = F, show_col_types = F) %>% .$X1
summary(tbr2_metrics)
max(tbr_best[[3]]$results$ROC)
(sum(tbr2_metrics >= max(tbr_best[[3]]$results$ROC)) + 1)/ (1000 + 1) 

tbr3_metrics <- read_csv("FigureData/Permutations/Tbr_MARS_perm_res.txt", col_names = F, show_col_types = F) %>% .$X1
summary(tbr3_metrics)
max(tbr_best[[6]]$results$ROC)
(sum(tbr3_metrics >= max(tbr_best[[6]]$results$ROC)) + 1)/ (1000 + 1)


plot_grid(
  data.frame(ROC = tbr1_metrics, method = rep("GLM", 1000)) %>% 
  ggplot(aes(x = ROC)) + 
  geom_histogram(fill = "#1177BB", color = "black") + 
  geom_vline(xintercept = tbr_best[[1]]$results$ROC, lty = 2, color = "red") +
  theme_bw() + 
  scale_x_continuous(limits = c(0,1)) +
  ggtitle("GLM"),
  data.frame(ROC = tbr2_metrics, method = rep("Boost", 1000)) %>% 
  ggplot(aes(x = ROC)) + 
  geom_histogram(fill = "#1177BB", color = "black") + 
  geom_vline(xintercept = max(tbr_best[[3]]$results$ROC), lty = 2, color = "red") +
  theme_bw() + 
  scale_x_continuous(limits = c(0,1)) +
  ggtitle("Boost"),
  data.frame(ROC = tbr3_metrics, method = rep("MARS", 1000)) %>% 
  ggplot(aes(x = ROC)) + 
  geom_histogram(fill = "#1177BB", color = "black") + 
  geom_vline(xintercept = max(tbr_best[[6]]$results$ROC), lty = 2, color = "red") +
  theme_bw() + 
  scale_x_continuous(limits = c(0,1)) +
  ggtitle("MARS"),
  nrow = 1
